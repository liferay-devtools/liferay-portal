...
JSONObjectDefinitionFileCheck | [Styling](styling_checks.md#styling-checks) | Enforces consistent ordering of `objectFields` in `*-object-definition.json` and `*object-definition.batch-engine-data.json` files (non-relationship fields first, followed by relationship fields, both sorted alphabetically by `externalReferenceCode`). |
