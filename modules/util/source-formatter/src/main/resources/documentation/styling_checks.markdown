...
GradleBlockOrderCheck | .gradle or .properties | Sorts logic in gradle build files. |
GradleBodyCheck | .gradle or .properties | Applies rules to enforce consistency in the body of gradle build files. |
GradleImportsCheck | .gradle or .properties | Sorts and groups imports in `.gradle` files. |
GradleIndentationCheck | .gradle or .properties | Finds incorrect indentation in gradle build files. |
GradleStylingCheck | .gradle or .properties | Applies rules to enforce consistency in code style. |
[GradleTaskCreationCheck](check/gradle_task_creation_check.markdown#gradletaskcreationcheck) | .gradle or .properties | Checks that a task is declared on a separate line before the closure. |
...
WhitespaceCheck | .cql, .css, .dtd, .expect, .gradle, .groovy, .properties, .scss, .sh, .soy, .sql, .tld, .ts, .tsx, Dockerfile or packageinfo | Finds missing and unnecessary whitespace. |
